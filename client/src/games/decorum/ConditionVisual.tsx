...
function Expression({ expr, house, ownerId, scoped = false }: { expr: ConditionExpression; house: HouseState; ownerId: string; scoped?: boolean }) {
  ...
  switch (expr.kind) {
    ...
    case "emptySlotCount": return <>{scope(expr.scope)}<Chip><Boxes size={17} />空物品格</Chip>{count(expr.comparison, expr.value)}</>;
    case "distinctCount": return <>{scope(expr.scope)}<Chip>{expr.target === "walls" ? <Paintbrush size={17} /> : <Boxes size={17} />}{expr.target === "walls" ? "牆面" : expr.target === "objects" ? "物品" : "牆面＋物品"}</Chip><Chip>不同{expr.trait === "color" ? "顏色" : "風格"}種數</Chip>{count(expr.comparison, expr.value)}</>;
