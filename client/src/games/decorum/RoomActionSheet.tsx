...
import { Paintbrush, Trash2, ArrowLeftRight, Check } from "lucide-react";
...
export function RoomActionSheet({ room, game, uid, initialMode, name, disabled, error, onClose, onConfirm }: {
  ...
  return <GameDialog title={`佈置${ROOM_LABELS[room.type]}`} onClose={onClose} className="decorum-dialog decorum-action-sheet">
    ...
    <div className="decor-tabs" aria-label="佈置類型">{[...OBJECT_TYPES, "paint" as const, ...(canMove ? ["roommate" as const] : [])].map((tab) => <button key={tab} aria-pressed={mode === tab} onClick={() => { setMode(tab); setPreview(null); }}>{tab === "paint" ? <><Paintbrush size={15} />牆面</> : tab === "roommate" ? <><ArrowLeftRight size={15} />換房</> : OBJECT_LABELS[tab]}</button>)}</div>
    ...
    {preview && <div className="decor-preview" role="status">{object ? <><FurnitureArt object={object} /><span>預覽：{preview.type === "decorAdd" ? "加入" : "換成"}{objectLabel(object)}</span></> : preview.type === "decorPaint" ? <><span className="paint-dot" style={{ background: paintColors[preview.color] }} /><span>預覽：{COLOR_LABELS[room.wallColor]} → {COLOR_LABELS[preview.color]}</span></> : preview.type === "decorRemove" ? <span>預覽：移除{OBJECT_LABELS[preview.objectType]}，留下空位。</span> : <span>預覽：搬進{ROOM_LABELS[room.type]}{preview.type === "decorRoommate" && preview.swapWith ? `，與 ${name(preview.swapWith)} 交換。` : "。"}</span>}</div>}
