...
export function RolePortrait({ role }: { role: Role }) {
  return <div className={`avalon-portrait portrait-${role}`} role="img" aria-label={`${ROLE_NAMES[role]}角色插畫`} />;
}
...
export function QuestMap({ game, concealLatest = false }: { game: Game; concealLatest?: boolean }) {
  return <section className="quest-map" aria-label="遠征任務版">
    ...
    <svg className="quest-landscape" viewBox="0 0 800 180" preserveAspectRatio="none" aria-hidden="true">
      <path d="M0 120 80 25 135 88 220 5 310 98 400 28 465 85 535 12 640 105 730 36 800 95V180H0Z" fill="#718779" opacity=".16" />
      <path d="M0 140Q100 80 200 135T400 130T600 138T800 120V180H0Z" fill="#bda573" opacity=".12" />
      <path d="M70 100Q170 45 240 90T400 95T565 85T730 100" fill="none" stroke="#d9b980" strokeWidth="2" strokeDasharray="6 7" opacity=".65" />
    </svg>
