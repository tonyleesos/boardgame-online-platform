...
React / TypeScript / Vite 桌遊平台，使用 Firebase 會員登入、Realtime Database 與 callable Cloud Functions。大廳提供 Avalon（5–10 人）、驚爆倫敦原版（4–8 人）、驚爆倫敦：危機進化（4–6 人）、同房異夢（2–4 人）、璀璨寶石（2–4 人）、教父風雲（6–12 人）與矮人礦坑 2（2–12 人）。除了同房異夢，其他遊戲皆支援 AI 練習與補位。

## 矮人礦坑 2 · 秘密身份與礦道冒險

大廳選「矮人礦坑 2」，建立朋友房或與 1–11 位 AI 練習。每場三輪，包含藍綠隊、破壞者、工頭、奸商及地質學家；支援色門、橋梁、梯子、工具破壞、陷阱、身份與手牌交換、偷竊及水晶計分。身份、手牌、偵查情報與累積金塊保持私人，第三輪結束公開總分。

手機與桌機使用單頁棋盤，拖曳／雙指縮放或按入口、目標定位；選道路、預覽位置後確認。點卡牌、玩家與規則開啟彈窗。玩家列直接顯示不能蓋路的工具與受困圖示；所有操作有約 3.2 秒的動畫與文字提示，AI 保留閱讀時間。使用原創 SVG 圖像與功能牌組，詳細範圍和測試方式見 [矮人礦坑實作說明](docs/SABOTEUR_IMPLEMENTATION.md)。
...
在大廳建立璀璨寶石房間，邀請 2–4 位朋友。房主可選基礎遊戲、城市、貿易站、東方之路或要塞；每次只啟用一種擴充。所有人準備後開始。以圖示選寶石，再確認拿取；點卡牌查看價格、永久折扣、黃金用量並購買或保留。手機直向以單一畫面呈現玩家、貴族、完整市場、寶石庫與自己的保留卡；點玩家查看完整收藏與寶石明細。
...
秘密心願以房間、家具、顏色、風格與數量圖示呈現，自己的條件會即時顯示 ✓／✗，完整文字直接列在圖卡下方。首次合租每人 4 項，其餘劇本每人 5 項，包含精確數量、空格、不同顏色／風格種數與跨房間條件。四人臥室需求可共存，所有 34 種發牌排列均有合法解法。圖卡只有本人、合法收件人或終局揭曉後可見。UI 不提供自由文字聊天。
...
結束時會優先彈出醒目的「好人陣營勝利／壞人陣營勝利」，關閉後查看圓桌與全員身份，也可隨時按「查看勝負」重開。隊伍表決使用圓形拇指贊成／反對票，秘密任務使用長方形聖杯／骷髏牌；戰報與任務地圖沿用相同任務圖示。AI 玩家名稱統一為隨機名稱加 `AI`（例如 `艾琳AI`），包括練習、補位及離席接手。
...
犯人在跳舞（`criminal-dance`）支援 3–8 人朋友房及 2–7 位 AI 練習，含基礎 32 張牌、少年選配、秘密同步交換、私人目擊情報及 5／10 分多輪比賽。卡牌採原創插圖，手機／桌機可直接操作，玩家行動保留約 3.2 秒動畫；詳見 [實作與驗證說明](docs/CRIMINAL_DANCE_IMPLEMENTATION.md)。

教父風雲：危情古巴（`mafia-de-cuba`）支援 6–12 人朋友房及單人 AI 練習，可指定自己或 AI 當教父。包含完整傳盒、秘密拿取、指控、清道夫進階模式與勝利結算；實作及私人資料時序見 [遊戲說明](docs/mafia-de-cuba.md)。單人練習由 5–11 名 AI 補位，AI 只使用公開資訊及自己的私人視野；指控仍是不確定的猜測。此遊戲暫時離線保留座位，明確離席則中止本局。
